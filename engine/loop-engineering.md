...
### Pre-flight: real-environment UI verification and credential capture

As part of the router judgment (before decomposition), decide whether the loop
will deliver **user-facing UI** that must be proven against a real running
environment (STG lane, staging, preview deploy). This is a first-step decision,
not an afterthought at the review gate — it determines whether the loop needs
browser access and credentials, and getting them **up front** is what lets the
loop then run unattended without a human-in-the-loop pause later.

When UI is in scope, plan to verify it the way a user experiences it — run the
workspace's browser QA (gstack `/qa` / `/browse`) against the real environment,
not only unit tests and `curl` (this is the Iteration Contract's browser-level
rule, and the checker's real-environment verification; the test-engineer and
delivery-reviewer roles reference this same expectation).

If that environment is gated by auth, **capture the credential once at loop
start**, before iterating:

- **Prefer reusing the user's already-authenticated browser session** instead of
  a login handoff. Read the live credential from the user's real browser — a
  bearer/JWT token from `localStorage`/`sessionStorage`, or an httpOnly session
  cookie — and inject it into the QA browser session (e.g. `browse
  cookie-import-browser <browser> --domain <host>`, or read a `localStorage`
  token and set it as an `Authorization` header / storage entry).
- Do this in the pre-flight (or the loop's first iteration) and record in
  `loop/state.md` that the credential/session is captured, so subsequent
  iterations and the checker reuse it and the loop runs unattended.
- If the credential genuinely cannot be obtained without the user (no logged-in
  session available, MFA challenge), that is a `BLOCKED` needing human input —
  surface it at the start, not after building.
- Never paste the captured token/cookie into committed files, logs, or the task
  record. Treat it as environment access material.
...
5. Verify with the owning repository's commands, choosing the cheapest check
   ...
   way a user experiences them (for web UI: browser-level checks against the
   real running environment via the workspace's browser QA — gstack `/qa` /
   `/browse` — not only unit tests and curl; playbook:
   `references/browser-verification.md`). Reuse the auth session/token captured
   in pre-flight so this runs without a human pause (see the Router's
   "Pre-flight: real-environment UI verification and credential capture").
