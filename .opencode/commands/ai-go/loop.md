...
1. Read the workspace's `AGENTS.md` and `engine/loop-engineering.md`.
2. **Route before executing.** Run the protocol's Loop Router: classify intent
   (mode), touched repositories (the workspace may contain multiple independent
   git repos — identify which this loop reads and which it writes), write risk,
   uncertainty, stop conditions, and needed roles. Honor an explicit `--mode`;
   `--readonly` forbids all code modification regardless of mode. If intent is
   genuinely ambiguous or the task implies high-risk operations, pause and ask
...
3. Resolve the loop task:
   ...
   - Otherwise treat `$ARGUMENTS` as a new goal: create the task directory with
     ...
     and loop/state.md (Status: RUNNING, Iteration: 0, plus the Router section:
     mode, write scope, goal, stop conditions, roles).
   - If `$ARGUMENTS` is empty, infer the goal from the current conversation and
     confirm it with the user before starting.
4. Follow the protocol's Default Strategy, adapted to the routed mode: read-only
   modes (analyze, review) never create branches or modify code; write modes
   (dev, fix, test) create a task branch or worktree in **each** repository they
   will modify, per that repository's own convention; design mode writes only
   task artifacts. Multi-repo work executes serially, slices ordered by
   cross-repo dependency (contracts first, then services, then clients). Pause
   for user confirmation when a design's blast radius is large.
5. Execute iterations per the Iteration Contract: get bearings, smoke-verify
   the current state first, one slice per iteration, verification evidence
   before claims, state.md appended every iteration, and a commit in every
   repository the slice modified — never treat the workspace root as one git
   repository.
